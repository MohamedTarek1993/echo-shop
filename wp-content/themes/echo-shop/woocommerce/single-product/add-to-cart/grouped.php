...
<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
					...
					switch ( $column_id ) {
						case 'quantity':
							...
							if ( ! $grouped_product_child->is_purchasable() || $grouped_product_child->has_options() || ! $grouped_product_child->is_in_stock() ) {
								...
								echo '<label for="' . esc_attr( 'quantity-' . $grouped_product_child->get_id() ) . '" class="screen-reader-text">' . esc_html__( 'Buy one of this item', 'echo-shop' ) . '</label>';
							} else {
	...
	<?php if ( $quantites_required && $show_add_to_cart_button ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button button xx alt"> <i class="bi bi-bag-fill"></i></button>
